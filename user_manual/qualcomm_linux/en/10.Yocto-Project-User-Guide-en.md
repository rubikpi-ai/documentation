...
:::note
 ...
 To set up a virtual machine (VM) running Ubuntu 22.04 on Microsoft Windows or Apple(®) macOS(®), refer to [Set up the Windows 11 development environment](./1.%20Get%20Started.md#win11env), [Set up an Ubuntu Server VM on an Arm64 Mac using UTM](./1.%20Get%20Started.md#arm64env), and [Set up an Ubuntu VM on an x86_64 Mac using UTM](./1.%20Get%20Started.md#x86env). Code compilation on a VM is a relatively slow process and may take several hours. It is recommended to use an Ubuntu host for compilation.
 :::
...
3. Flash the image. For details, refer to [Flash images](./1.%20Get%20Started.md#flashimages).
