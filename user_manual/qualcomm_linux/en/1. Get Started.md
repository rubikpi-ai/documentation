...
2. Run the following commands in the Ubuntu terminal to install Python 3.10.2. In the [Linux kernel packaging](./1.%20Get%20Started.md#linuxkernelpack) process, the `python3.10` command is used. If you have installed Python 3.10.2 or later, skip this step.
...
1. On the Ubuntu host, visit the [RUBIK Pi official website](https://www.thundercomm.com/rubik-pi-3/en/docs/image) to download the cross-compilation toolchain for RUBIK Pi 3, then run the following command to decompress it.
   ...
   For details about eSDK compilation, refer to the [Yocto Project User Guide](./10.%20Yocto%20Project%20User%20Guide.md#chap10) section.
   :::
...
:::note
 ...
 You can only compile the Qualcomm Linux build using WSL. To flash the software on RUBIK Pi 3, see [1.3.3.1 Flashing images on a Windows system](./1.%20Get%20Started.md#flashwin).
:::
...
To sync, build, and flash Qualcomm Linux, see [Yocto Project User Guide](./10.%20Yocto%20Project%20User%20Guide.md#chap10) and [Linux kernel](./1.%20Get%20Started.md#LinuxKernel).
:::
...
On the Qualcomm platform, drivers and device trees related to display, graphics, camera, video, and other components are separated from the main Linux kernel source and maintained in independent repositories. Follow the [Yocto Project User Guide](./10.%20Yocto%20Project%20User%20Guide.md#chap10) to download the Yocto project source code. You can then navigate to the *layers/meta-qcom-hwe* directory, where the *.bb* files provide information about the specific code repositories.
...
1. Run the following command to set up the cross-compilation environment. For details, refer to [Install cross-compilation tools](./1.%20Get%20Started.md#Installcross-compilationtools).
