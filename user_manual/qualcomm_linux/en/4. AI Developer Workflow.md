...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| <p>[Install Qualcomm Neural Processing Engine SDK](./4.%20AI%20Developer%20Workflow.md#installNPESDK)</p><p>[Setup Qualcomm Neural Processing SDK](./4.%20AI%20Developer%20Workflow.md#SetNPESDK)</p> | <p>[Model Conversion](./4.%20AI%20Developer%20Workflow.md#modelconversion)</p><p>[Model Quantization](./4.%20AI%20Developer%20Workflow.md#modelquantization)</p> | [Model deployment](./4.%20AI%20Developer%20Workflow.md#modeldeployment) | [C/C++](https://docs.qualcomm.com/bundle/publicresource/topics/80-63442-2/api.html) |
...
| ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ---------------------------------- |
| [Direct Download](./4.%20AI%20Developer%20Workflow.md#directdownload)          | No prerequisites for download                                 | v2.22.6 (SDK updated everyquarter) |
| [Qualcomm Package Manager](./4.%20AI%20Developer%20Workflow.md#Qpackmanager) | A valid Qualcomm ID is needed. Qualcomm Package Manager tool. | SDK updated every month            |
...
If installed via the direct download method, aaaaa is the path to the unzipped SDK.
...
```python
sudo bash aaaaa/bin/check-linux-dependency.sh
```
...
aaaaa/bin/check-python-dependency
```
...
```shell
aaaaa/bin/x86_64-linux-clang/snpe-onnx-to-dlc --input_network ~/models/
...
```python
aaaaa/bin/x86_64-linux-clang/snpe-dlc-quant --input_dlc ~/models/
...
```python
aaaaa/bin/x86_64-linux-clang/snpe-dlc-graph-prepare --input_dlc ~/
...
```python
aaaaa/bin/x86_64-linux-clang/snpe-dlc-info -i ~/models/
...
```python
aaaaa/bin/x86_64-linux-clang/snpe-net-run --container ~/models/
...
For RUBIK Pi 3, use the below artifacts from *aaaaa/lib/aarch64-oe-linux-gcc11.2* and *aaaaa/bin/aarch64-oe-linux-gcc11.2*.

| **File**                                       | **Source location**                         |
| ---------------------------------------------- | ------------------------------------------- |
| snpe-net-run                                   | aaaaa/bin/aarch64-oe-linux-gcc11.2  |
| libSNPE.so                                     | aaaaa/lib/aarch64-oe-linux-gcc11.2/ |
| libSnpeHtpPrepare.so                           | aaaaa/lib/aarch64-oe-linux-gcc11.2  |
| libSnpeHtpV68Stub.so                           | aaaaa/lib/aarch64-oe-linux-gcc11.2  |
| libSnpeHtpV68Skel.so                           | aaaaa/lib/hexagon-v68/unsigned      |
...
```bash
scp aaaaa/bin/aarch64-oe-linux-gcc11.2/snpe-net-run root@[ipaddr]:/opt/
scp aaaaa/lib/aarch64-oe-linux-gcc11.2/libSNPE.so root@[ip-addr]:/opt/
scp aaaaa/lib/aarch64-oe-linux-gcc11.2/libSnpeHtpV68Stub.so root@[ip-addr]:/opt/
scp aaaaa/lib/aarch64-oe-linux-gcc11.2/libSnpeHtpPrepare.so root@[ip-addr]:/opt/
scp aaaaa/lib/hexagon-v68/unsigned/libSnpeHtpV68Skel.so root@[ip-addr]:/opt/
...
| **Setup**                                                                                                                                                                                                                                                        | **Customize**                                                                                                                                                                                                                                              | **Run Inference**                                                                                                | **API Reference**                                                                    |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
|<p> [Install AI Engine Direct](./4.%20AI%20Developer%20Workflow.md#InstallAIEngineDirect)</p><p>[Set up Qualcomm AI Engine Direct](./4.%20AI%20Developer%20Workflow.md#SetupQualcommAIEngineDirect) </p>| <p>[Model conversion and quantization](./4.%20AI%20Developer%20Workflow.md#Modelconversionquantization)</p><p>[Model compilation](./4.%20AI%20Developer%20Workflow.md#Modelcompilation) </p>|<p>[Deploy model](./4.%20AI%20Developer%20Workflow.md#deploymodel)</p> | <p>[C/C++](https://docs.qualcomm.com/bundle/publicresource/topics/80-63442-50/api.html)</p> |
...
If the host machine uses Windows or macOS operating system software, install a virtual machine following the steps provided in [Set up the Windows 11 development environment](./1.%20Get%20Started.md#win11env), [Set up an Ubuntu Server VM on an Arm64 Mac using UTM](./1.%20Get%20Started.md#arm64env), and [Set up an Ubuntu VM on an x86_64 Mac using UTM](./1.%20Get%20Started.md#x86env). Subsequent steps must be run in the virtual machine running Ubuntu 22.04 LTS.
:::
...
| **Download Method**                                                                                                          | **Prerequisites**                                             | **Version available**              |
| ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ---------------------------------- |
| [Direct Download](/4.%20AI%20Developer%20Workflow.md#directdown)          | No prerequisites for download.                                | v2.22.6 (SDK updated everyquarter) |
| [Qualcomm Package Manager](/4.%20AI%20Developer%20Workflow.md#Qpackmg) | A valid Qualcomm ID is needed. Qualcomm Package Manager tool. | SDK updated every month.           |
...
*bbbbb* refers to the installed AI Engine Direct SDK root path.&#x20;

* If downloaded via the direct download method, set *bbbbb* to the downloaded and unzipped SDK root directory.
...
* Install Python dependencies with `bbbbb/bin/check-python-dependency`

* Install Ubuntu packages with `sudo bash bbbbb/bin/check-linux-dependency.sh`
...
```shell
python bbbbb/bin/x86_64-linux-clang/qnn-onnx-converter --
...
```bash
bbbbb/bin/x86_64-linux-clang/qnn-onnx-converter --input_network ~/
...
```shell
bbbbb/bin/x86_64-linux-clang/qnn-model-lib-generator -c ~/models/
...
```bash
bbbbb/bin/x86_64-linux-clang/qnn-net-run --model ~/models/libs/
x86_64-linux-clang/libinception_v3.so --backend bbbbb/lib/x86_64-
...
For RUBIK Pi 3, use artifacts from *bbbbb/bin/aarch64-oe-linux-gcc11.2* and *bbbbb/lib/aarch64-oe-linux-gcc11.2.*

| **File**                       | **Source location**                                         |
| ------------------------------ | ----------------------------------------------------------- |
| qnn-net-run                    | bbbbb/bin/aarch64-oe-linux-gcc11.2              |
| libQnnHtp.so                   | bbbbb/lib/aarch64-oe-linux-gcc11.2/libQnnHtp.so |
| libQnnCpu.so                   | bbbbb/lib/aarch64-oe-linux-gcc11.2/libQnnCpu.so |
| libQnnGpu.so                   | bbbbb/lib/aarch64-oe-linux-gcc11.2/libQnnGpu.so |
| libQnnHtpPrepare.so            | bbbbb/lib/aarch64-oe-linux-gcc11.2              |
| libQnnHtpV68Stub.so            | bbbbb/lib/aarch64-oe-linux-gcc11.2              |
| libinception_v3_quantized.so | \~/models/libs/aarch64-oe-linux-gcc11.2                     |
| libinception_v3.so            | \~/models/libs/aarch64-oe-linux-gcc11.2                     |
| libQnnHtpV68Skel.so            | bbbbb/lib/hexagon-v68/unsigned                  |
| libqnnhtpv68.cat               | bbbbb/lib/hexagon-v68/unsigned                  |
| libQnnSaver.so                 | bbbbb/lib/hexagon-v68/unsigned                  |
| libQnnSystem.so                | bbbbb/lib/hexagon-v68/unsigned                  |

```bash
scp bbbbb/bin/aarch64-oe-linux-gcc11.2/qnn-* root@[ip-addr]:/opt/
scp bbbbb/lib/aarch64-oe-linux-gcc11.2/libQnn*.so root@[ip-addr]:/opt/
scp bbbbb/lib/hexagon-v68/unsigned/* root@[ip-addr]:/opt/
