...
2. 在 Ubuntu 终端中使用下面命令，安装 Python 3.10.2，在 [Linux kernel 的打包](1.%20快速开始.md#Linuxkernelpack)过程中用到了`python3.10`命令，如果已经有 Python 3.10.2 或更高版本，请跳过这个步骤。
...
1. 在 Ubuntu 主机上，访问 [RUBIK Pi 3 官方网站](https://www.thundercomm.com/rubik-pi-3/cn/docs/image)，下载 RUBIK Pi 3 的交叉编译工具链，下载完成后使用下面命令解压。
   ...
   关于 eSDK 的编译可参考 [Yocto 工程编译指南](./10.%20Yocto%20工程编译指南.md#yocto)章节。
   :::
...
:::note
只能使用 WSL 编译 Qualcomm Linux 软件。要在 RUBIK Pi 3 上刷写软件，请参阅 [Windows 烧录方法](./1.%20快速开始.md#flashwin)。
:::
...
:::note
 ...
 要同步、编译和刷写 Qualcomm Linux，参见 [Yocto 工程编译指南](./10.%20Yocto%20工程编译指南.md#yocto)、[Linux kernel ](./1.%20快速开始.md#linuxkernel)等章节
:::
...
高通平台将显示、图形、摄像头、视频等相关的驱动和设备树从 linux kernel 源码中分离出来成独立的仓库，可根据 [Yocto 工程编译指南](./10.%20Yocto%20工程编译指南.md#yocto) 章节下载 Yocto 工程源码，通过查看 *layers/meta-qcom-hwe* 目录下的 *.bb* 文件提供的信息访问具体的代码仓库，以查看摄像头的设备树为例：
...
1. 运行下面的命令设置交叉编译环境，具体请参考 [安装交叉编译工具](./1.%20快速开始.md#installcrosstool)。
