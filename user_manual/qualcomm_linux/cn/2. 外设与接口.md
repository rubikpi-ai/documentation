...
* 编译程序：
  ...
  * 交叉编译，具体可参考 [交叉编译工具使用方法](./1.%20快速开始.md#usecrosstool)章节：
...
* Raspberry Pi High Quality Camera（IMX477/M12 Mount）[购买链接](https://www.raspberrypi.com/products/raspberry-pi-high-quality-camera/)
  ...
  在 [RUBIK Pi 3 QIM](./5.%20Qualcomm%20IM%20SDK.md#ChapQIM) 官方演示中，IMX477 摄像头使用了 WS1053516 镜头。
...
1. 检查摄像头模块连接。
   ...
   请参阅 [摄像头排线安装](./2.%20外设与接口.md#cameracable)。
...
在配对远程设备之前，[运行蓝牙查询扫描](./2.%20外设与接口.md#startscan)以确保远程设备可用。
...
开始之前，请按照[为低功耗蓝牙 GATT 功能设置设备](./2.%20外设与接口.md#SetforGATTDev)中的说明设置设备。
...
:::note
>
> 要获取远程设备的蓝牙地址，请运行[蓝牙低功耗 GATT 扫描](./2.%20外设与接口.md#startBTGATTscan)。
:::
...
`<Flags>` 是特征的标志值。有关具体值的信息，请参阅[标志值](./2.%20外设与接口.md#flagvalue)。
...
`<Flags>` 是描述符的标志值。有关具体值的信息，请参阅[标志值](./2.%20外设与接口.md#flagvalue)。
...
开始之前，将所需服务添加为[主要服务](./2.%20外设与接口.md#addprimary)。
...
要读取特征值，请从 `menu gatt` 菜单执行以下操作：
  ...
  1. [选择所需的属性](./2.%20外设与接口.md#selectattr)。
  2. 通过运行以下命令读取所选属性：
  ```
read 0
  ```
...
关于 HOGP 功能，请参阅[执行蓝牙 HOGP 功能](./2.%20外设与接口.md#performBTHOGP)。
...
2. 在蓝牙无线鼠标的 DUT 上运行[低能耗扫描 ](./2.%20外设与接口.md#startBTGATTscan)。

3. [将 DUT 连接](./2.%20外设与接口.md#conenctRMT)至蓝牙无线鼠标。
...
```
  ...
  要在源中执行 **Transport submenu** 功能，请参阅[执行蓝牙 A2DP 发送功能](./2.%20外设与接口.md#BTADPsource)。
  要在接收端中执行 **Transport submenu** 功能，请参阅[执行蓝牙 A2DP 接收端功能](./2.%20外设与接口.md#BTADPsink)。
...
要在接收端中执行 **Player submenu** 功能，请参阅[执行蓝牙 A2DP 接收端功能](./2.%20外设与接口.md#BTADPsink)。

* [执行蓝牙 A2DP 发送功能](./2.%20外设与接口.md#BTADPsource)
  用户可以使用 `bluetoothctl` 菜单、 `menu transport` 子菜单和 `paplay` 命令执行蓝牙 A2DP 发送功能。

* [执行蓝牙 A2DP 接收端功能](./2.%20外设与接口.md#BTADPsink)
...
开始之前，请设置设备并转至[设置设备的 A2DP 功能](./2.%20外设与接口.md#SetA2DP)中所述的所需菜单。
...
根据需要配置 DUT 的 HFP [客户端](./2.%20外设与接口.md#configHFPclient)或[音频网关](./2.%20外设与接口.md#ConfigHFPgtw)功能。

**配置 DUT 的 HFP 客户端功能**
<a id="configHFPclient"></a>

在为 HFP 功能进行[初始设备设置](./2.%20外设与接口.md#HFPinit)后，请对 HFP 客户端执行以下操作：
...
sh-5.1#
```
关于 HFP 客户端功能，请参阅[执行蓝牙 HFP 客户端功能](./2.%20外设与接口.md#performHFPclient)。

**配置 DUT 的 HFP 音频网关功能**
<a id="ConfigHFPgtw"></a>

在为 HFP 功能进行[初始设备设置](./2.%20外设与接口.md#HFPinit)后，请对 HFP 音频网关执行以下操作：
...
关于 HFP 音频网关功能，请参阅[执行蓝牙 HFP 音频网关功能](./2.%20外设与接口.md#performHFPgtw)。

* [执行蓝牙 HFP 客户端功能](./2.%20外设与接口.md#performHFPclient)
  可以使用 `bluetoothctl` 菜单和 `ofono/test` 工具执行 HFP 客户端功能。

* [执行蓝牙 HFP 音频网关功能](./2.%20外设与接口.md#performHFPgtw)
...
开始之前，请[设置您的设备](./2.%20外设与接口.md#HFPinit)，并且[配置 DUT 以实现 HFP 客户端功能](./2.%20外设与接口.md#configHFPclient)。
...
开始之前，请[设置您的设备](./2.%20外设与接口.md#HFPinit)，并且[配置 DUT 以实现 HFP 音频网关功能](./2.%20外设与接口.md#ConfigHFPgtw)。
...
该命令提供 **obexctl** 的主菜单。要执行 OPP 服务器和客户端功能，请参阅[执行蓝牙 OPP 服务器功能](./2.%20外设与接口.md#perOPPclient)和[执行蓝牙 OPP 客户端功能](./2.%20外设与接口.md#perOPPserv)。
...
* [执行蓝牙 OPP 服务器功能](./2.%20外设与接口.md#perOPPclient)
  用户可以使用 **obexctl** 主菜单中提供的命令来验证蓝牙 OPP 服务器功能。

* [执行蓝牙 OPP 客户端功能](./2.%20外设与接口.md#perOPPserv)
...
要以服务器角色接收文件，执行以下操作：
  ...
  1. 配对 DUT 和远程设备。相关说明，可参见[与远程蓝牙设备配对](./2.%20外设与接口.md#pairRMT)。
  2. 从远程设备启动到 DUT 的连接。
...
开始之前，请按照[设置设备的蓝牙 OPP 功能](./2.%20外设与接口.md#setOPP)中的说明设置设备。
...
* 配对 DUT 和远程设备。相关说明，可参见[与远程蓝牙设备配对](./2.%20外设与接口.md#pairRMT)。
...
要执行 FTP 客户端功能，请参阅[执行蓝牙 FTP 客户端功能](./2.%20外设与接口.md#perFTP)。
...
开始之前，请按照[设置设备的蓝牙 FTP 功能](./2.%20外设与接口.md#setFTP)中的说明设置设备。
...
开始之前，[将当前文件夹](./2.%20外设与接口.md#changefolder)更改为所需文件的文件夹。
...
该命令提供 **obexctl** 的主菜单。要执行 PBAP 服务器和客户端功能，请参阅[执行蓝牙 PBAP 服务器功能](./2.%20外设与接口.md#perPBAPserv)和[执行蓝牙 PBAP 客户端功能](./2.%20外设与接口.md#perPBAPclient)。
...
[执行蓝牙 PBAP 服务器功能](./2.%20外设与接口.md#perPBAPserv)
用户可以使用 **obexctl** 主菜单中提供的命令验证蓝牙 PBAP 服务器功能。

[执行蓝牙 PBAP 客户端功能](./2.%20外设与接口.md#perPBAPclient)
...
:::note
>
> 在 BlueZ 堆栈中，无法直接测试或验证 PBAP 服务器功能，因为您无法在服务器上创建联系人。因此，您必须在 DUT 上创建 PBAP 目录。关于创建 PBAP 目录的更多信息，请参阅 [PBAP 目录示例](./2.%20外设与接口.md#PBAPdir)。
:::
...
开始之前，请按照[设置设备的蓝牙 PBAP 功能](./2.%20外设与接口.md#setPBAP)中的说明设置设备。
...
要提取电话簿，请执行以下操作：
  ...
  1. [连接 DUT 和远程设备](./2.%20外设与接口.md#PBAPRMT)。
  2. [选择所需的电话簿](./2.%20外设与接口.md#selectobj)。
  3. 通过运行以下命令来提取所需的电话簿：
...
该命令提供 **obexctl** 的主菜单。要执行 MAP 服务器和客户端功能，请参阅[执行蓝牙 MAP 服务器功能](./2.%20外设与接口.md#perMAPserv)和[执行蓝牙 MAP 客户端功能](./2.%20外设与接口.md#perMAPclient)。
...
* [执行蓝牙 MAP 服务器功能](./2.%20外设与接口.md#perMAPserv)
  用户可以使用 **obexctl** 主菜单中提供的命令验证蓝牙 MAP 服务器功能。

* [执行蓝牙 MAP 客户端功能](./2.%20外设与接口.md#perMAPclient)
...
:::note
>
> 在 BlueZ 堆栈中，无法直接测试或验证 MAP 服务器功能，因为您无法在设备上发送或接收蜂窝消息。因此，您必须在 DUT 上创建一个 MAP 目录。关于创建 MAP 目录的更多信息，请参阅[示例 MAP 目录](./2.%20外设与接口.md#sampleMAPdir)。
:::
...
开始之前，请按照[设置设备的蓝牙 MAP 功能](./2.%20外设与接口.md#setBTMAP)中的说明设置设备。
...
要列出文件夹中的消息，请执行以下操作：
  ...
  1. [连接 DUT 和远程设备](./2.%20外设与接口.md#conrmt)。
  2. 将当前目录更改为预期目录。
