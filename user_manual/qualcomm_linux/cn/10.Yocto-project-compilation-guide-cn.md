...
:::note
>
> 要在 Microsoft Windows 或 Apple(®) macOS(®) 上搭建运行 Ubuntu 22.04 的虚拟机 (VM)，请参阅 [Windows 11 开发环境搭建](./1.%20快速开始.md#setwin)、 [Mac 开发环境搭建（Arm64）](./1.%20快速开始.md#setmac)、 [Mac 开发环境搭建（x86\_64）](./1.%20快速开始.md#set86)。VM 上的代码编译是一个较慢的过程，可能需要几个小时。 建议使用 Ubuntu 主机进行编译。
:::
...
3. 参考 [镜像烧录](./1.%20快速开始.md#flashimages) 小结进行镜像烧录。
