...
TODO:"Canonical Ubuntu for Qualcomm Platforms"原来标注开发中，现在不用标注了吗？
...
* Debian 13：基于GNOME 48 桌面系统，系统功能完善，丰富的软件包资源，有更多的桌面应用，更好的使用体验，较多的开发资料，适合追求开发便捷的开发者。
  ...
  ![](<images/Screenshot From 2025-02-20 06-36-21.png>)
...
1. 安装 WinUSB 驱动（若已经安装可忽略该步骤）
    1. 卸载该设备的其他驱动程序。确保未安装 Qualcomm USB 驱动程序等驱动程序。您不应该在设备管理器的 COM 端口下看到该设备。

        * 如果您在 COM 端口下看到该设备，请按照屏幕截图卸载 QUD。确保已选中“删除此设备的驱动程序软件”。
        TODOCS: "您不应该在设备管理器的 COM 端口下看到该设备。"->"该设备不应该出现在设备管理器的 COM 端口下。"，并合到"如果您在 xxx"这句前面？还有就是，"该设备"指什么，是否需要更明确说下

        ![](images/20250602-214517.jpg)

    2. 使用以下说明/屏幕截图安装 Microsoft WinUSB。
...
```
TODOCS: 上面命令中 <pathToQDL>\QDL 这个反斜杠是否保留？预览出来是有的，如果命令不需要就删除吧
...
1. 执行下面命令安装 libusb 和 libxml2 等（若已经安装，可忽略该步骤）。
TODOCS: "等"具体指啥工具？括号说已经安装就忽略，所以还是说清楚这个命令是安装些啥，不然用户不知道自己到底安装了没
...
> 进行 provision 后，UFS 中存储的一些信息会丢失，如 SN号、以太网MAC地址等。
>
> 根据主机架构将 *QDL_Linux_x64* 或 *QDL_Linux_ARM* 目录下的 qdl 拷贝到 *provision* 目录
TODOCS: 这是 provision 前进行的操作吗？是的话前面加一句，"配置前"？
:::
```shell
...
> 进行 provision 后，UFS 中存储的一些信息会丢失，如 SN号、以太网MAC地址等。
>
> 根据主机架构将 *QDL_Mac_x64* 或 *QDL_Mac_ARM* 目录下的 qdl 拷贝到 *provision* 目录
:::
```shell
./qdl prog_firehose_ddr.elf provision_ufs_1_3.xml
```
...
:::note
Ubuntu24.04 V1.0.0 版本镜像未提供 linux kernel 源码，linux kernel 源码将在后续发布的稳定版本中提供。
:::
