...
为了提升开发者的查阅效率，RUBIK Pi 文档现已集成 **Google NotebookLM** 功能。该工具将我们静态的技术资料库（包括数据手册、SDK 指南和常见问题解答）转化为可交互的知识库。
...
第二步：提交问题
在 NotebookLM 界面底部的对话框中输入您的技术问题。系统将自动分析 RUBIK Pi 的全套文档并生成回答。
...
| 查询示例 | 推荐提问示例 |
|----------|--------------|
| 硬件资料 | 列出 Rubik Pi 3 的 40-pin GPIO 引脚定义表。|
| 系统安装 | 如何将 Linux 镜像烧录到 Rubik Pi 3？（支持 SD 卡启动吗？） |
...
- **专属知识库**： 该助手基于 RUBIK Pi 官方软硬件手册训练，确保回答的针对性。
