...
custom_edit_url: https://github.com/rubikpi-ai/documentation/blob/main/docs-cn/docs/rubik-pi-3-user-manual/1.1.2/4.ai-developer-workflow.md
---
...
- 编译并优化来自第三方 AI 框架的模型，以便在魔方派 3 上高效运行。例如，可以将 TensorFlow 模型导出为 TFLite 模型。
...
导航到[ AI Hub Model Zoo](https://aihub.qualcomm.com/iot/models)，访问适用于 魔方派 3 的模型.
...
提交编译作业时，必须选择设备或芯片组以及目标 runtime 才能编译模型。魔方派 3 支持 TFLite runtime。
...
- 提交的每项作业都可以在 AI Hub 门户中进行审核。提交编译作业时，将提供优化模型的可下载链接。然后，该优化模型可以部署在魔方派 3 等本地开发设备上。

下方所示为从 [AI Hub documentation](https://aihub.qualcomm.com/iot/models) 中获取的所述工作流程的示例。在本示例中，会将源自 PyTorch 的MobileNet V2 预训练模型上传到 AI Hub，并编译为优化的 TFLite 模型以便在魔方派 3上运行。
...
对于 魔方派 3，使用路径 *$\{SNPE_ROOT\}/lib/aarch64-oe-linux-gcc11.2* 和 *$\{SNPE_ROOT\}/bin/aarch64-oe-linux-gcc11.2*.*&#x20;*&#x4E2D;的以下文件。
...
对于 x86 架构，可使用 Clang 编译工具链将 C++ 图编译为 .so 库。对于 魔方派 3 等 Linux 嵌入式设备，必须使用适当的编译工具链 (aarch64-oe-linux-gcc11.2)。
...
对于 魔方派 3，使用路径 *$\{QNN_SDK_ROOT\}/bin/aarch64-oe-linux-gcc11.2* &#x548C;*$\{QNN_SDK_ROOT\}/lib/aarch64-oe-linux-gcc11.2.&#x20;*&#x4E2D;的文件。
...
在基于 Arm 架构的 CPU 上执行模型时，qnn-net-run 需要使用模型 .so（对于 魔方派 3 目标设备，.so 必须使用 aarch64-oe-linux-gcc11.2 工具链进行交叉编译）、后端 .so 库和输入列表来运行推理，从而生成输出。
