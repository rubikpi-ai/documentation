...
* 使用 Python 的 periphery 库控制 GPIO，需先在魔方派 3 中使用下面命令安装 python-periphery：
...
* 使用 Python 的 smbus 库控制 I2C，需先在魔方派 3 中使用下面命令安装 smbus 库：
...
* 使用 Python 的 spidev 库进行 SPI 通信，需先在魔方派 3 中使用下面命令安装spidev 库：
...
* 使用 Python 的 serial 库进行 UART 通信，需先在魔方派 3 中使用下面命令安装 serial 库：
...
USB 2.0 接口默认为主机模式，作为设备模式时需要手动执行命令切换，如下为一种切换方式，在魔方派 3 中输入下面命令，将 魔方派 3 模拟为 U 盘：
...
在魔方派 3 中可使用 `gstreamer` 命令对摄像头进行操作，操作前需输入下面命令对摄像头进行设置：
...
在 STA 模式（Station）下，设备可连接到一个已经存在的无线网络，以便访问网络资源或互联网。在魔方派 3 中输入如下面命令进行连接：
...
* 验证 AP
  ...
  3. 选择 AP 并输入在魔方派 3 设备上为 AP 配置的相应 `wpa_passphrase`，然后进行连接。&#x20;
  ...
  4. 要验证连接，在魔方派 3 设备的终端 中 ping 移动设备的 IP 地址。
...
* 其他 Demo
  ...
  在魔方派 3 的 */usr/src/rubikpi-bt-demo/3rdparty/3rdparty/embedded/bsa\_examples/linux* 目录下，存放了所有 蓝牙 相关的源码，可以按需要进行编译和查看，编译方法如下：
...
在魔方派 3 中输入下面命令，改变 gstreamer、paplay 等命令的音频输出源头：
...
* 在魔方派 3 中输入下面命令测试蓝牙设备播放，测试需要开启多个终端。
...
魔方派 3 风扇会根据当前的 CPU 温度自动调整转速，同时也可在魔方派 3 中使用下面命令进行手动控制转速，0 和 255 分别表示风扇的最低和最高速度。
...
目前 魔方派 3 暂未支持固态硬盘的自动挂载，可在魔方派 3 中执行下面命令对固态硬盘手动挂载：
