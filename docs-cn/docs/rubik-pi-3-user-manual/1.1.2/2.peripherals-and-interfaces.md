...
custom_edit_url: https://github.com/rubikpi-ai/documentation/blob/main/docs-cn/docs/rubik-pi-3-user-manual/1.1.2/2.peripherals-and-interfaces.md
---
...
魔方派 3 适配了 WiringRP（基于高性能 GPIO 编程库 WiringPi），推荐使用 WiringRP 来对 GPIO 进行控制和编程。关于 WiringRP 详细信息可访问 https://github.com/rubikpi-ai/WiringRP 查看。
...
* 使用 Python 的 periphery 库控制 GPIO，需先在 魔方派 3 中使用下面命令安装 python-periphery：
...
* 编译程序：
  ...
  * 交叉编译，具体可参考 [交叉编译工具使用方法](./1.get-started.md#usecrosstool)章节：
    ...
    若使用了交叉编译，需将 gpio 传输到 魔方派 3 中，如使用 ADB 传输：
...
魔方派 3 适配了 WiringRP（基于高性能 GPIO 编程库 WiringPi ）。推荐使用 WiringRP 对 I2C 进行控制和编程。关于 WiringRP 详细信息可访问 https://github.com/rubikpi-ai/WiringRP 查看。
...
* &#x5C06;*&#x20;i2c.py&#x20;*&#x4F20;输到 魔方派 3 中，如使用 ADB 传输。
...
* 使用 Python 的 smbus 库控制 I2C，需先在 魔方派 3 中使用下面命令安装 smbus 库：
...
* 将 *i2c.py&#x20;*&#x4F20;输到 魔方派 3中，如果使用 ADB 传输，命令如下：
...
* 编译程序：
  ...
  * 交叉编译，具体可参考 [交叉编译工具使用方法](./1.get-started.md#usecrosstool) 章节：
    ...
    若使用的交叉编译，需要将 *i2c&#x20;*&#x4F20;输到 魔方派 3 中，如果使用 ADB 传输，命令如下：
...
魔方派 3 适配了 WiringRP（基于高性能 GPIO 编程库 WiringPi），推荐使用 WiringRP 对 SPI 进行控制和编程。关于 WiringRP 详细信息可访问 https://github.com/rubikpi-ai/WiringRP 查看。
...
* &#x5C06;*&#x20;spi.py&#x20;*&#x4F20;输到 魔方派 3 中，如使用 ADB 传输。
...
* 使用 Python 的 spidev 库进行 SPI 通信，需先在 魔方派 3 中使用下面命令安装spidev 库：
...
* 编译程序：
  ...
  * 交叉编译，具体可参考 [交叉编译工具使用方法](./1.get-started.md#usecrosstool)章节：
    ...
    ```
     ...
     若使用交叉编译，需要将 spi 传输到 魔方派 3 中，如果使用 ADB 传输，命令如下：
...
魔方派 3 适配了 WiringRP（基于高性能 GPIO 编程库 WiringPi），推荐使用 WiringRP 对 UART 进行控制和编程。关于 WiringRP 详细信息可访问 https://github.com/rubikpi-ai/WiringRP 查看。
...
* 在 魔方派 3 上开启两个终端，将 8 号引脚和 10 号引脚使用杜邦线短接，分别执行下面命令，接收端会回显发送端的内容：
...
* 使用 Python 的 serial 库进行 UART 通信，需先在 魔方派 3 中使用下面命令安装 serial 库：
...
* 编译程序：
  ...
  * 交叉编译，具体可参考[交叉编译工具使用方法](./1.get-started.md#usecrosstool)章节：
    ...
    若使用交叉编译，需要将 *uart&#x20;*&#x4F20;输到 魔方派 3 中，如果使用 ADB 传输，命令如下：
...
USB 2.0 接口默认为主机模式，作为设备模式时需要手动执行命令切换，如下为一种切换方式，在 魔方派 3 中输入下面命令，将 魔方派 3 模拟为 U 盘：
...
目前，魔方派 3与[外设兼容列表](https://www.thundercomm.com/rubik-pi-3/cn/docs/peripheral-compatibility-list)中列出的相机模块兼容。下表列出了每个相机模块支持的分辨率。
...
* HQ Camera（IMX477/M12 Mount）
  ...
  在 [魔方派 3 QIM](./5.qualcomm-im-sdk.md#ChapQIM) 官方演示中，IMX477 摄像头使用了 WS1053516 镜头。
...
* Camera Module 3 (IMX708) 
  ...
  目前 魔方派 3 暂时只支持标准版 Module 3 摄像头，不支持广角（Wide）、夜光（NoIR）版本。当前软件版本暂不支持 Module 3 摄像头的 AF 自动对焦功能。
  :::

### 摄像头排线安装
<a id="cameracable"></a>

魔方派 3 支持的摄像头 FPC 为 22 pin，0.5mm 间距，厚度 0.3±0.05mm。兼容的附件请参考[外设兼容列表](https://www.thundercomm.com/rubik-pi-3/cn/docs/peripheral-compatibility-list)。
...
在 魔方派 3 中可使用 `gstreamer` 命令对摄像头进行操作，操作前需输入下面命令对摄像头进行设置：
...
若电视支持 CEC 功能，可在 魔方派 3 中使用下面命令控制电视音量的加减：
...
魔方派 3 拥有 1 个 USB Type-C 接口的 DisplayPort (DP)，如下图 5。
...
魔方派 3 上搭载了 AP6256 Wi-Fi 模块，支持 Wi-Fi 5 和蓝牙 5.2。
...
在 STA 模式（Station）下，设备可连接到一个已经存在的无线网络，以便访问网络资源或互联网。在 魔方派 3 中输入如下面命令进行连接：
...
* 验证 AP
  ...
  3. 选择 AP 并输入在 魔方派 3 设备上为 AP 配置的相应 `wpa_passphrase`，然后进行连接。&#x20;
  ...
  4. 要验证连接，在 魔方派 3 设备的终端 中 ping 移动设备的 IP 地址。
  ...
  也可以在连接设备的 Settings 中验证 Wi-Fi 连接状态。例如，要获取连接到 魔方派 3 AP 的移动设备的 IP 地址，执行以下步骤：
...
* 其他 Demo
  ...
  在 魔方派 3 的 */usr/src/rubikpi-bt-demo/3rdparty/3rdparty/embedded/bsa\_examples/linux* 目录下，存放了所有 蓝牙 相关的源码，可以按需要进行编译和查看，编译方法如下：
...
* 在 魔方派 3 中输入下面命令，查看已分配的 PCM 流列表：
...
在 魔方派 3 中输入下面命令，改变 gstreamer、paplay 等命令的音频输出源头：
...
* 在 魔方派 3 中输入下面命令测试蓝牙设备播放，测试需要开启多个终端。
...
与魔方派 3兼容的风扇请参考[外设兼容列表](https://www.thundercomm.com/rubik-pi-3/cn/docs/peripheral-compatibility-list)。

:::warning
>
> 在一些高负载或高性能的场景下使用 魔方派 3 ，需要使用散热措施来保证设备性能稳定，否则会因为 CPU 温度过高而带来降频等性能影响。
:::
...
魔方派 3 风扇会根据当前的 CPU 温度自动调整转速，同时也可在 魔方派 3 中使用下面命令进行手动控制转速，0 和 255 分别表示风扇的最低和最高速度。
...
魔方派 3 上有一个由 PMIC (Power Management IC) 驱动的 RGB 的三色灯，默认情况下其中的绿色灯被设置为了心跳灯用来反馈系统的运行情况和 CPU 负载情况）。
...
可在 魔方派 3 中使用下面命令查看网络连接情况，如下图所示，已经成功分配 IP，说明网络连接成功：
...
接入 RTC 电池， 如下图所示，并将时间写入 魔方派 3 的系统硬件时钟，可在 魔方派 3 完全断电后，仍对 魔方派 3 的系统时间进行保存。

![](images/20250314-155435.jpg)

在 魔方派 3 中将系统时间写入系统硬件时钟方法如下：
...
魔方派 3 提供一个标准 M.2 插槽，用于连接 NVMe 存储，可安装 2280 规格（22\*80mm）M.2 SSD 硬盘。M.2 插槽最大支持 PCIe Gen 3 x 2。M.2 接口最大对外输出能力为 3.3V 2A，可单独控制开关。
...
目前 魔方派 3 暂未支持固态硬盘的自动挂载，可在 魔方派 3 中执行下面命令对固态硬盘手动挂载：
