...
custom_edit_url: https://github.com/rubikpi-ai/documentation/blob/main/docs-cn/docs/rubik-pi-3-user-manual/1.1.2/10.yocto-project-user-guide.md
---
...
本章节介绍如何使用 Yocto 工程编译出可以在魔方派 3 上烧录和运行的镜像。
...
本部分使用安装在 Ubuntu 主机配置中的 Repo 工具，从 [魔方派 manifest](https://github.com/rubikpi-ai/rubikpi-manifest) 下载 git 存储库和其他属性。Repo 工具使用 `repo init` 命令下载 manifest。
...
2. 下载预编译二进制文件:
    ```bash
    cd src/vendor/thundercomm/prebuilt/BP-BINs
    git lfs pull
    cd -
    ```
...
在 [魔方派 3 官方网站](https://www.thundercomm.com/rubik-pi-3/cn/docs/image) 中已经上传了预先编译好的 eSDK，下面说明 eSDK 具体的编译方法。
